...
- Die bisherige `StudentListWidgetContent`-Komponente zeigte Verhaltenssymbole, Notiz-Indikatoren und negative Punkt-Aktionen öffentlich. Der produktive `studentlist`-Renderer verwendet auf diesem Branch stattdessen die isolierte `PublicStudentListWidget`: eindeutige kurze Namen, nur positive Punkte, große Touch-Plusfläche, gezieltes Undo nach bewusstem Plus. Die vertrauliche alte Implementierung bleibt unaufgerufen im Quellcode; ein Browser-Sicherheits- und Persistenztest steht noch aus.
