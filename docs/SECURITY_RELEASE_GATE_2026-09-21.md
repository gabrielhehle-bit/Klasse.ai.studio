...
- Default-deny **all** outgoing Gemini requests even when an API key is set; the operator must explicitly set KLASSIO_AI_EXTERNAL_ENABLED=true after a documented privacy review. Even when enabled, student grade/portfolio actions and image uploads remain blocked.
...
7. External AI now defaults to OFF. Do not set KLASSIO_AI_EXTERNAL_ENABLED=true until remaining free-text actions, processor terms and the lawful workflow have been reviewed by the school/data controller; regular-expression masking cannot guarantee anonymity.
