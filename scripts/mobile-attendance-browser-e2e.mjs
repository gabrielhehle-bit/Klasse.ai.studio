...
async function main() {
  ...
  try {
    ...
    await waitFor(client, 'new encrypted vault',
      'document.body?.innerText.toLowerCase().includes("lokalen datentresor einrichten")', 30000);
    ...
    if (result.phone !== 360 || result.docWidth > 365) throw new Error('Phone layout overflows the viewport.');
    if (!result.header || result.header.width > 360 || result.header.height > 260) throw new Error('Attendance header is oversized on the phone.');
    ...
    if (topButtons.some(x => x.r && x.r.right > 363)) throw new Error('Phone toolbar action is clipped.');
    await saveScreenshot(client);
    console.log('KLASSIO mobile attendance synthetic browser layout passed.');
