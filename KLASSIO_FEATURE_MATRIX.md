...
| Notizen | 🟡 | Eigener Hauptbereich in der Sidebar. Er öffnet direkt die zentrale Notizerfassung: allgemeine Klassen-Notiz oder Kind auswählen, Kategorie wählen (Notiz, Beobachtung/Verhalten, Erfolg/Stärke, Elternkontakt, Klassenjournal), Text eingeben und speichern. Chronik, Suche und Filter bleiben klassenlokal; Beobachtungsstatus ist nur noch ein sekundärer Tab. PR #108: `Diktieren` nutzt auf unterstützten Chromium-Versionen lokale On-Device-Spracherkennung, ansonsten Browser-Fallback; Mikrofonrechte sowie `network`-, Mikrofon- und Sprachpaketfehler werden robust behandelt und erneutes Starten bleibt möglich. Automatisierte Tests und Browser-E2Es grün; reale Mikrofon-/Diktat-Abnahme auf klassio.at nach Deployment noch offen. |
| Kernnavigation | ✅ | Heute, Klasse, Notizen, Planung, Leistungen und Unterricht sind direkte Hauptbereiche. Notizen ist bewusst ohne Umweg erreichbar; Unterricht bleibt eigener Hub. Weitere Bestandswerkzeuge bleiben vollständig über Sidebar, Hubs bzw. `Mehr` erreichbar. Regressionstests sichern Vollständigkeit und Reihenfolge. |
...
| Schülerliste | 🟡 | Suche, Filter, Liste/Karten/Karte, Import, Dossier und Bearbeiten/Löschen bleiben erhalten. Die frühere Notiz-/Interaktionsaktion wurde bewusst aus der Klassenliste entfernt; Notizen werden zentral im Hauptbereich `Notizen` erfasst. Alter und Alterssortierung sind kalendergenau; Suche umfasst auch SV-Nummer. Reimporte erkennen bestehende Kinder und aktualisieren nur Stammdaten, ohne pädagogische Daten zu überschreiben; Sokrates-Metadaten werden übernommen. Löschen bereinigt die zugehörigen personenbezogenen Klassendaten vollständig, während Kassenbuchungen entkoppelt erhalten bleiben. Reale Browser-Abnahme offen. |
...
| Jahresplanung | 🟡 | PR #53: Vollbild, Lehrplan-Drawer, KI-Vorschläge, Druck und mehrere Themen bleiben erhalten. PR #103: bestehende Planungen öffnen zuerst eine Übersicht und erst über `Bearbeiten` den deutlich größeren Editor; der Wochenplan kann eine freie KW-/Fach-Zelle sicher ergänzen, belegte Zellen werden nie überschrieben. PR #106: der Editor nutzt nahezu die vollständige Bildschirmbreite ohne feste 1400-px-Grenze. Automatisierte Tests inkl. Planungsregressionen grün; reale Touch-/Druck-/Excel-Abnahme offen. |
| Wochenplanung | 🟡 | PR #55: Vollbild, Raster, Schnellplanung, Tagesfortschritt, Termine, Duplizieren und Schüler-Aufgabenblatt nutzen alle 10 Stunden-Slots; Stunde 9/10 erfinden keine Standardzeiten. PR #103: `Einheit planen` nutzt eine große Arbeitsfläche; geplante Stunden öffnen zuerst eine übersichtliche Zusammenfassung und wechseln erst über `Bearbeiten` in den Editor. Wochenplan → Jahresplan ergänzt nur freie Zellen derselben KW/Fachspalte. PR #106: `Einheit planen` nutzt nahezu die vollständige Bildschirmbreite ohne feste 1500-px-Grenze. Keine Migration, kein Reset vorhandener Planungsdaten. Automatisierte Tests inkl. Planungsregressionen grün; reale Touch-/Druck-/Excel-Abnahme offen. |
...
| E-Mail-Einmalcode-Login | 🟡 | In `main` integriert und auf dem World4You-Server aktiviert. Jede gültige E-Mail kann ein persönliches Klassio-Konto nutzen; Schulidentität bleibt davon getrennt. 30-Tage-Session, Rate-Limits und Codeablauf bleiben erhalten. SMTP ist produktiv konfiguriert (`emailLoginEnabled=true`); ein echter Zustelltest über das World4You-Postfach bleibt als Live-Abnahme offen. |
| Schulregister Österreich | ✅ | PR #86 + #101: persistentes Register mit stabiler Schul-ID, exakter Schul-Domain und allen 9 Bundesländern. VS Oberau / `vsfoa.vobs.at` ist initialer Seed. Unbekannte Schulen können direkt in `Konto & Schulmail` eine Verifizierung anfordern; private Mailanbieter und Provider-Sammeldomains wie `vobs.at` bleiben ausgeschlossen. Admin-Freigabe/Ablehnung ist in Klassio integriert und im realen Chrome-E2E geprüft. |
| Schulverifizierungs-Admin | ✅ | PR #101: neue Anfragen lösen eine Admin-Mail aus; konfigurierte Admin-E-Mail-Konten sehen unter `Einstellungen → Konto & Schulmail` die Schulverwaltung und können Anfragen freigeben oder ablehnen. Die anfragende Lehrperson wird über das Ergebnis informiert; kein Admin-Secret wird an den Browser ausgeliefert. Realer Chrome-E2E grün. |
| Lehrerzimmer / Kollegium | 🟡 | Beiträge, Fragen, Antworten, @Erwähnungen und Kollegium bleiben strikt an die verifizierte Schul-ID gebunden. Private Klassio-Konten erhalten keinen schulweiten Zugriff. PR #101 aktiviert die Schulidentität nach Admin-Freigabe automatisch; Live-Abnahme auf `klassio.at` nach aktuellem Deployment bleibt offen. |
| Gemeinsame Klassen / Teamteaching | 🟡 | Jede Lehrperson nutzt ein eigenes E-Mail-Konto und einen eigenen lokalen Tresor/Geräteschlüssel; Klassen werden nur explizit für Mitglieder derselben verifizierten Schule freigegeben. Owner/Editor/Viewer, AES-GCM-256-Klassensnapshot, RSA-OAEP-Geräteschlüssel, Revisions-/Konfliktschutz und Geräte-Nachfreigabe sind implementiert. Der reale Zwei-Konto-Chrome-E2E ist auch auf dem PR-#101-Stand grün. Live-Abnahme auf `klassio.at` nach aktuellem Deployment bleibt offen. |
...
| Bereich | Status | Nachweis / Rest |
|---|---:|---|
| World4You Runtime grundsätzlich | ✅ | Linux-vServer läuft produktiv mit Nginx/systemd/HTTPS. Produktion läuft über `klassio.service` auf `127.0.0.1:3100`, Nginx bedient `klassio.at`; interne App-Ports sind nicht öffentlich freigegeben. |
| World4You Release-Paket | ✅ | Deployments werden commitgebunden aus dem aktuellen GitHub-`main` nach `/srv/klassio/releases/` gebaut; `current`/`previous` ermöglichen kontrollierte Aktivierung und Rollback. |
| World4You Abschluss-Staging | 🟡 | Staging ist derzeit nicht die operative Arbeitsgrundlage. Verbindlich bleibt GitHub-`main`; produktive Releases werden erst nach grünen Tests und Browser-E2E ausgerollt. |
| Finaler Browser-Test | 🟡 | PR #101: realer Chrome-E2E für Schulverifizierung ist grün (bestehende Klasse → Schul-Antrag → Admin-Mail → Admin-Freigabe → automatische Schulidentität → bestehende Klasse bleibt erhalten). Zwei-Konto-Teamteaching-E2E ist ebenfalls grün. Weitere Module wie Druck/PDF/PPTX, Offline/PWA und echte Altbackup-Datei bleiben separat abzunehmen. |
| Merge nach `main` | ✅ | Reconciliation ist abgeschlossen. PR #100 und PR #101 sind in `main` integriert; `main` ist die vollständige und einzige verbindliche Entwicklungsbasis. |
