...
| Bereich | Status | Nachweis / Rest |
|---|---:|---|
| World4You Runtime grundsätzlich | 🟡 | Ein Linux vServer S ist für Klassio bestellt und noch in Bereitstellung. Der bisher untersuchte Shared-Webspace ist nicht mehr die geplante finale Klassio-Runtime. Draft-PR #80 bereitet Nginx → 127.0.0.1:3100, systemd, Secrets außerhalb von Git, Healthcheck und Rollback vor; reale Serverabnahme beginnt erst nach Bereitstellung. |
...
| World4You Abschluss-Staging | 🔴 | Nach vServer-Bereitstellung das exakte aktuelle Reconciliation-Artefakt deployen, Commitmarker prüfen, lokalen und öffentlichen `/api/health`-Check durchführen, DNS/HTTPS verifizieren und erst danach den realen Gesamt-Walkthrough starten. |
