...
test('Klasse, Planung and Leistungen hubs expose their remaining legacy tools', () => {
  ...
  assertContainsAll(source('PlanungHub.tsx'), ['planungszentrale', 'wochenplanung', 'jahresplanung', 'stunden', 'materialien', 'canva', 'uebergabemappe']);
  assert.match(source('PlanungHub.tsx'), /setPage\(['"]vertretung['"]\)/);
