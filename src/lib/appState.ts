...
export function syncActiveClass(state: AppState): AppState {
  ...
  const updatedClass = {
    ...
    hausuebungen: state.hausuebungen ? JSON.parse(JSON.stringify(state.hausuebungen)) : [],
...
export function normalizeAppState(raw: any): AppState {
  ...
  if ((!parsed.classes || !Array.isArray(parsed.classes) || parsed.classes.length === 0)
    ...
    const defaultClass: any = {
      ...
      hausuebungen: parsed.hausuebungen || [],
  ...
  if (parsed.classes && Array.isArray(parsed.classes)) {
    parsed.classes = parsed.classes.map((c: any) => {
      ...
      return {
        ...
        hausuebungen: c.hausuebungen ?? (c.id === parsed.activeClassId ? parsed.hausuebungen : undefined) ?? [],
  ...
  if (activeClass) {
    ...
    parsed.hausuebungen = activeClass.hausuebungen || [];
...
export function switchClassState(prev: AppState, id: string): AppState {
  ...
  return {
    ...
    hausuebungen: targetClass.hausuebungen ? JSON.parse(JSON.stringify(targetClass.hausuebungen)) : [],
