...
import { getKW, getCurrentSchuljahr } from './utils';
...
export function normalizeAppState(raw: any): AppState {
  ...
  if (activeClass) {
    ...
    parsed.klassenglas_missions = activeClass.klassenglas_missions;
    parsed.klassenglas_completed_missions = activeClass.klassenglas_completed_missions || [];
...
export function switchClassState(prev: AppState, id: string): AppState {
  prev = syncActiveClass(prev);
      ...
      return {
        ...prev,
        saAssessments: targetClass.saAssessments ? structuredClone(targetClass.saAssessments) : {},
        scheduleAnalysis: targetClass.scheduleAnalysis ? structuredClone(targetClass.scheduleAnalysis) : undefined,
        klassenglas_missions: targetClass.klassenglas_missions ? structuredClone(targetClass.klassenglas_missions) : undefined,
        klassenglas_completed_missions: targetClass.klassenglas_completed_missions ? [...targetClass.klassenglas_completed_missions] : [],
