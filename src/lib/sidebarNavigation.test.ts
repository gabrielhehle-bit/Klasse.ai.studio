...
const items = [
  'dashboard', 'klasse', 'verhalten', 'planung', 'leistungen', 'cockpit', 'ki-helfer',
  'schueler', 'anwesenheit', 'noten',
...
test('Klassio core navigation includes Notizen as a direct main area', () => {
  ...
  assert.deepEqual(result.daily.map(i => i.id), ['dashboard', 'klasse', 'verhalten', 'planung', 'leistungen', 'cockpit', 'ki-helfer']);
