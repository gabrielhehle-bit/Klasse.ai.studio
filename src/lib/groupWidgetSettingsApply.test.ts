...
test('group settings chosen before the very first add are not masked by old hidden defaults', () => {
  ...
  assert.match(source, /type === "groups" &&[\s\S]{0,100}Array\.isArray\(targetWidget\.settings\?\.groups\)[\s\S]{0,100}targetWidget\.settings\.groups\.length > 0/);
  assert.match(source, /type === "groups" && !useOld[\s\S]{0,120}\? \{ \.\.\.\(widget\.settings \|\| \{\}\), \.\.\.groupPreset \}/);
