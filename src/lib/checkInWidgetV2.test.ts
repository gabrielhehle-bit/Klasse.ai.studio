...
test('Widget 1: picker gear saves class-local defaults separately from installed instances', () => {
  assert.match(picker, /aria-label="Ich bin da! einstellen"/);
  assert.match(picker, /aria-label="Ich bin da! hinzufügen"/);
