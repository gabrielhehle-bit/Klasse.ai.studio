...
export default function Sek1StudentDossier({ studentId, onBack }: { studentId: string; onBack: () => void }) {
  ...
  const grades = sek1GradeEntries(app, studentId, semester);
  const notes = getStudentNotes(app, studentId);
  const gradebookNotes = sek1GradebookComments(app, studentId);
  ...
  return (
    <article className="mx-auto w-full max-w-5xl space-y-5">
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5">
        ...
        {gradebookNotes.length > 0 && <div className="mt-4"><p className="text-sm font-extrabold text-[var(--text)]">Notizen aus der Notenmappe</p><ul className="mt-2 space-y-2">{gradebookNotes.map(item => <li key={`${item.fach}-${item.semester}`} className="rounded-xl bg-[var(--surface2)] p-3 text-sm text-[var(--text)]"><strong>{item.fach} · {item.semester}. Semester:</strong> {item.text}</li>)}</ul></div>}
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5">
        ...
        {grades.length ? <ul className="mt-3 space-y-2">{grades.map(g => <li key={g.fach} className="flex items-center justify-between rounded-lg bg-[var(--surface2)] p-3 text-sm text-[var(--text)]"><span>{g.fach}</span><strong>{g.quelle === 'direkt_eingetragen' ? 'Direkt erfasst: ' : 'Rechnerischer Wert: '}{g.wert}</strong></li>)}</ul> : <p className="mt-3 text-sm text-[var(--text2)]">Für dieses Semester liegen noch keine Bewertungen vor.</p>}
