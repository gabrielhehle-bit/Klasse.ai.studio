...
interface WochenplanExcelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (rows: WochenplanImportRow[], targetKW: number, mode: 'overwrite' | 'merge') => void;
  activeKW: number;
  app: AppState;
}

export default function WochenplanExcelModal({
  isOpen,
  onClose,
  onImport,
  activeKW,
  app
}: WochenplanExcelModalProps) {
  const [file, setFile] = useState<File | null>(null);
  const [loading, setLoading] = useState(false);
  const [parseResult, setParseResult] = useState<WochenplanImportResult | null>(null);
  const [targetKW, setTargetKW] = useState<number>(activeKW);
  ...
  const handleFileChange = async (selectedFile: File) => {
    ...
    try {
      const res = await parseWochenplanExcel(selectedFile);
      setParseResult(res);
    } catch (err: any) {
      setParseResult({
        success: false,
  ...
  const handleConfirmImport = () => {
    ...
    // Filter out rows that are examples
    const actualRows = parseResult.rows.filter(r => !r.isExample);
    onImport(actualRows, targetKW, importMode);
    onClose();
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/70">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900">Excel-Import: Wochenplan</h2>
              <p className="text-xs text-slate-500 font-medium">
                Excel-Datei einlesen, prüfen und in die Wochenplanung übernehmen
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="bg-emerald-50/70 border border-emerald-200 rounded-xl p-3 sm:p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start sm:items-center gap-2.5">
              ...
              <div>
                <span className="text-xs font-bold text-emerald-950 block sm:inline">Passende Vorlage benötigt?</span>
                <span className="text-xs text-emerald-800/80 sm:ml-1.5 font-medium">Lade die vorbereitete Excel-Vorlage mit allen Spalten herunter.</span>
              </div>
            </div>
            <button
              onClick={() => generateWochenplanTemplate(app, activeKW)}
          ...
          {!EXCEL_IMPORT_DISABLED && !parseResult && (
            ...
            >
              ...
              <p className="text-sm font-bold text-slate-800 mb-1">
                {file ? file.name : 'Excel-Datei (.xlsx / .xls) hier ablegen oder auswählen'}
              </p>
              <p className="text-xs text-slate-400 font-medium">
                Die Datei wird vollständig lokal im Browser geprüft. Keine Daten verlassen dein Gerät.
              </p>
            </div>
          )}
          ...
          {loading && (
            <div className="py-10 text-center flex flex-col items-center justify-center">
              ...
              <p className="text-xs font-bold text-slate-600">Excel-Datei wird geprüft...</p>
            </div>
          )}
          ...
          {parseResult && !parseResult.success && (
            <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-bold text-rose-900 mb-1">Datei konnte nicht importiert werden</h4>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-emerald-950">
                      {parseResult.validRows} Unterrichtseinheiten erkannt
                    </h4>
                    <p className="text-[11px] text-emerald-800 font-medium">
                      Tage: {parseResult.detectedDays.join(', ') || 'Keine'} (aus {file?.name})
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-slate-300 rounded-lg p-2 text-xs font-bold text-slate-800 focus:ring-2 focus:ring-emerald-500 outline-none"
                  >
                    {Array.from({ length: 53 }, (_, i) => i + 1).map(kw => (
                      <option key={kw} value={kw}>
                        KW {kw} {kw === activeKW ? '(Aktuelle Auswahl)' : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="flex items-center gap-1.5 text-xs text-slate-700 font-bold cursor-pointer bg-white px-2.5 py-1.5 border border-slate-200 rounded-lg flex-1">
                      <input
                        type="radio"
                        name="importMode"
                      ...
                      />
                      <span>Ergänzen (Lücken füllen)</span>
                    </label>
                    <label className="flex items-center gap-1.5 text-xs text-slate-700 font-bold cursor-pointer bg-white px-2.5 py-1.5 border border-slate-200 rounded-lg flex-1">
                      <input
                        type="radio"
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-100/90 text-slate-700 font-bold sticky top-0 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="py-2 px-2.5">Std.</th>
                        <th className="py-2 px-2.5">Fach</th>
                        <th className="py-2 px-2.5">Thema / Inhalt</th>
                        <th className="py-2 px-2.5">Lernziel / Methode</th>
                        <th className="py-2 px-2.5">Hausübung</th>
                      </tr>
                    </thead>
                      ...
                      {parseResult.rows.map((row, idx) => (
                        ...
                        >
                          <td className="py-1.5 px-2.5 font-bold text-slate-800 whitespace-nowrap">
                            {row.tag} {row.isExample && <span className="text-[10px] text-amber-700">(Beispiel)</span>}
                          </td>
                          <td className="py-1.5 px-2.5 text-slate-600 whitespace-nowrap">{row.stunde}.</td>
                          ...
                          <td className="py-1.5 px-2.5 text-slate-600 max-w-xs truncate">
                            {row.lernziel || <span className="text-slate-300">-</span>}
                          </td>
                          <td className="py-1.5 px-2.5 text-slate-600 max-w-xs truncate">
                            {row.housework || <span className="text-slate-300">-</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-t border-slate-100 bg-slate-50/70">
          ...
          >
            <span>{parseResult?.validRows || 0} Einheiten importieren</span>
            <ArrowRight size={14} />
          </button>
        </div>
      </motion.div>
