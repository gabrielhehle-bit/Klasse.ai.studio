...
export default function Sek1PrintCenter() {
  ...
  const studentGrades = currentStudent ? getStudentGradeSummary(app,currentStudent.id,subjects,semester) : null;
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] print:block">
      ...
      <article id="sek1-print-content" className="min-w-0 rounded-2xl border border-[var(--border)] bg-white p-5 text-slate-900 shadow-sm print:rounded-none print:p-0">
        ...
        {template==='dossier' && <div className="mt-4 space-y-4">{currentStudent?<>
          ...
          <section><h4 className="font-bold">Bewertungen · {semester}. Semester</h4>{studentGrades?.subjectGrades.length?<ul>{studentGrades.subjectGrades.map(g=><li key={g.subject} className="text-sm">{g.subject}: {g.value.toFixed(2)} (rechnerisch)</li>)}</ul>:<p className="text-sm">Keine berechenbaren Werte.</p>}
            {subjects.flatMap(f=>['1','2'].flatMap(sem=>{const comment=app.noten?.[currentStudent.id]?.[f]?.[sem]?.freitext?.trim();return comment?[{key:`${f}-${sem}`,label:`${f}, ${sem}. Sem.`,text:comment}]:[]})).map(comment=><p key={comment.key} className="mt-1 text-sm">{comment.label}: {comment.text}</p>)}</section>
