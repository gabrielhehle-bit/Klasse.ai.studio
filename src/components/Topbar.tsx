...
import { 
  ...
  Bold, Italic, Save, Sliders, GraduationCap
} from 'lucide-react';
...
const Topbar = memo(({ title, onMenuClick, actions, className }: TopbarProps) => {
  ...
  const availableClasses = app?.classes && app.classes.length > 0 
    ? app.classes 
    : (app?.klassenbezeichnung ? [{ id: app.activeClassId || 'default', name: app.klassenbezeichnung, stufe: app.stufe || 1, klassenvorstand: app.klassenvorstand }] : []);
    ...
    <header className={`flex flex-col sticky top-0 z-[100] topbar no-print print:hidden ${className || ''}`}>
      ...
      <div className="bg-[var(--surface-card,var(--surface))]/95 backdrop-blur-xl border-b border-[var(--border-default,var(--border))] py-2.5 px-3 sm:px-6 shadow-xs">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-2 sm:gap-4 w-full">
          ...
          {/* Linker Bereich: Mobile Hamburger & Aktive Klasse */}
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Klassen-Badge & Schnell-Wechsler */}
            <div className="relative">
              <button
                type="button"
                onClick={() => setShowClassDropdown(!showClassDropdown)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--surface-subtle,var(--surface2))] hover:bg-[var(--surface-muted)] border border-[var(--border-default,var(--border))] rounded-2xl text-xs font-black text-[var(--text-primary)] transition-all cursor-pointer shadow-2xs active:scale-95"
                title="Klasse wechseln oder neue Klasse hinzufügen"
              >
                <GraduationCap size={15} className="text-emerald-600 shrink-0" />
                <span className="max-w-[120px] sm:max-w-[160px] truncate">{currentClassName}</span>
                <ChevronDown size={13} className={`text-[var(--text-secondary)] transition-transform duration-200 ${showClassDropdown ? 'rotate-180' : ''}`} />
              </button>
              ...
              {showClassDropdown && (
                <>
                  ...
                  />
                  <div className="absolute left-0 top-full mt-2 w-72 bg-[var(--surface-card,var(--surface))] rounded-2xl shadow-xl border border-[var(--border-default,var(--border))] p-2 z-50 animate-in fade-in zoom-in-95 duration-100 text-left">
                    <div className="px-3 py-1.5 border-b border-[var(--border-default,var(--border))]/50 flex items-center justify-between text-[0.625rem] font-black uppercase tracking-wider text-[var(--text-secondary)]">
                      <span>Deine Klassen</span>
                      <span>{(availableClasses.length || 1)} aktiv</span>
                    </div>
                      ...
                      {availableClasses.map((c: any) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              if (!isActive) switchClass(c.id);
                              setShowClassDropdown(false);
                            }}
                            className={`w-full text-left px-3 py-2 rounded-xl text-xs flex items-center justify-between transition-all cursor-pointer ${
                              isActive 
                                ? 'bg-emerald-500/10 text-emerald-600 font-black border border-emerald-500/20' 
                                : 'text-[var(--text-primary)] hover:bg-[var(--surface-subtle,var(--surface2))] font-bold'
                            }`}
                          >
                            <div className="flex items-center gap-2">
                              <GraduationCap size={14} className={isActive ? 'text-emerald-600' : 'text-[var(--text-secondary)]'} />
                              <div>
                                <div className="leading-tight">{c.name}</div>
                                <div className="text-[0.625rem] font-medium text-[var(--text-secondary)]">
                                  {c.stufe ? `${c.stufe}. Stufe • ` : ''}{c.schueler?.length || 0} Schüler:innen
                                </div>
                              </div>
                            </div>
                    ...
                    <div className="border-t border-[var(--border-default,var(--border))]/50 pt-1.5 mt-1 space-y-1">
                      <button
                        type="button"
                        onClick={() => {
                          setShowClassDropdown(false);
                        ...
                        className="w-full text-left px-3 py-2 rounded-xl text-xs font-black text-emerald-600 hover:bg-emerald-500/10 transition-colors flex items-center gap-2 cursor-pointer"
                      >
                        <Plus size={14} />
                        <span>+ Neue Klasse hinzufügen</span>
                      </button>
                    </div>
                  </div>
                </>
