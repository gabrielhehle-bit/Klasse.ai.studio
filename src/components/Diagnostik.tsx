...
const Diagnostik: React.FC = () => {
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className={`print:hidden rounded-[2rem] border p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4 ${
        diagnosticDataQuality.invalid > 0
          ? 'bg-amber-50 border-amber-200'
          : 'bg-emerald-50 border-emerald-200'
      }`}>
        <div className="flex items-start gap-3">
          <div className={`p-2.5 rounded-2xl ${diagnosticDataQuality.invalid > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
            {diagnosticDataQuality.invalid > 0 ? <AlertTriangle size={20} /> : <CheckCircle2 size={20} />}
          </div>
          <div>
            <h2 className="font-black text-slate-900">Datenqualität der aktuellen Klasse</h2>
            <p className="text-sm font-semibold text-slate-600 mt-1">
              {diagnosticDataQuality.valid} von {diagnosticDataQuality.total} Erhebungen sind eindeutig einem Kind zugeordnet und vollständig auswertbar.
            </p>
            <p className="text-xs text-slate-500 mt-1">
              Nur gültige Erhebungen fließen in Klassenverläufe und Schülerdossiers ein. Schwellenwerte erzeugen einen Prüfhinweis, keine automatische Förderdiagnose.
            </p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2 text-xs font-black">
          <span className="px-3 py-2 rounded-xl bg-white text-emerald-700 border border-emerald-100">{diagnosticDataQuality.valid} gültig</span>
          {diagnosticDataQuality.warnings > 0 && (
            <span className="px-3 py-2 rounded-xl bg-white text-amber-700 border border-amber-100">{diagnosticDataQuality.warnings} Altdaten-Hinweise</span>
          )}
          {diagnosticDataQuality.invalid > 0 && (
            <span className="px-3 py-2 rounded-xl bg-white text-rose-700 border border-rose-100">{diagnosticDataQuality.invalid} nicht berücksichtigt</span>
          )}
        </div>
      </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50/70 via-emerald-50/20 to-white p-6 rounded-[2rem] border border-emerald-100/80 shadow-[0_4px_25px_rgba(16,185,129,0.02)] space-y-4 hover:shadow-[0_12px_30px_rgba(16,185,129,0.05)] transition-all duration-300">
          ...
          <div className="space-y-1">
            <div className="flex items-baseline gap-1">
              <span className="text-[2rem] leading-none font-black text-slate-900 tracking-tight">
                {diagnosticDataQuality.valid}
              </span>
              <span className="text-[0.6875rem] font-bold text-slate-400">Einträge</span>
            </div>
            <p className="text-[0.6875rem] font-semibold text-slate-400">
              Gültige Einträge aus {tests.length} aktiven Testverfahren.
            </p>
          </div>
