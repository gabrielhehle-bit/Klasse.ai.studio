...
type Props = { items: InkItem[]; active: boolean; onChange: (items: InkItem[]) => void; onDone: () => void };

/** Vector ink belongs to the classroom, independent of widget positions. */
export function BoardInk({ items, active, onChange, onDone }: Props) {
  ...
  return <>
    <svg viewBox="0 0 1600 900" preserveAspectRatio="none" aria-label="Gemeinsame Schreib- und Zeichenfläche"
      ...
      onPointerDown={e => {
        ...
        if (tool === 'text') { setTextAt(point(e)); setText(''); return; }
        e.currentTarget.setPointerCapture(e.pointerId); pointer.current = e.pointerId;
        const p = point(e);
        stroke.current = { id: crypto.randomUUID(), color, width, points: [p, p] }; setDraft(stroke.current);
      }}
      onPointerMove={e => {
      ...
      {[...items, ...(draft ? [draft] : [])].map(item => <g key={item.id}
        onPointerDown={e => { if (active && tool === 'erase') { e.stopPropagation(); commit(items.filter(i => i.id !== item.id)); } }}>
        {item.text !== undefined ? <text x={item.points[0][0]} y={item.points[0][1]} fill={item.color} fontSize={32} fontFamily="sans-serif">{item.text}</text>
          : <><polyline points={item.points.map(p => p.join(',')).join(' ')} fill="none" stroke={item.color} strokeWidth={item.width} strokeLinecap="round" strokeLinejoin="round" />
            {active && tool === 'erase' && <polyline points={item.points.map(p => p.join(',')).join(' ')} fill="none" stroke="transparent" strokeWidth={Math.max(18, item.width)} pointerEvents="stroke" />}</>}
      </g>)}
    </svg>
    {active && <div className="absolute bottom-3 left-3 right-3 z-[21000] flex flex-wrap items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 text-slate-800 shadow-xl" role="toolbar" aria-label="Zeichenwerkzeuge"
      onKeyDown={e => { if (e.key === 'Escape') { cancel(); onDone(); } }}>
      {([['pen', 'Stift'], ['text', 'Text'], ['erase', 'Radierer']] as const).map(([id, label]) => <button key={id} className={button + (tool === id ? ' ring-2 ring-indigo-600' : '')} aria-pressed={tool === id} onClick={() => setTool(id)}>{label}</button>)}
      <label className="flex items-center gap-2 text-sm">Farbe <input aria-label="Stiftfarbe" type="color" value={color} onChange={e => setColor(e.target.value)} className="w-11 h-11" /></label>
      ...
      <button className={button} disabled={!items.length} onClick={() => setConfirmClear(true)}>Schrift & Zeichnung löschen</button>
    ...
    {active && confirmClear && <div role="dialog" aria-label="Schrift und Zeichnung löschen" className="absolute top-4 inset-x-4 z-[22000] p-4 bg-white text-slate-900 rounded-xl border shadow-xl">
      <p className="mb-3">Alle Striche und Texte auf dieser Fläche löschen? Die Widgets bleiben erhalten.</p>
      <div className="flex gap-2"><button className={button} onClick={() => { commit([]); setConfirmClear(false); }}>Schrift & Zeichnung löschen</button><button className={button} onClick={() => setConfirmClear(false)}>Abbrechen</button></div>
    </div>}
  </>;
}
