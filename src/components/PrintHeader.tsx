      ...
      <div className="flex justify-between items-start">
        <div className="w-1/3">
          ...
          <h1 className="text-[12pt] font-black text-black leading-tight">
            {app?.schulName?.trim() || 'Schule nicht angegeben'} {app?.schulkennzahl ? `(SKZ: ${app.schulkennzahl})` : ''} <br />
            <span className="text-[10pt] font-bold">Klasse: {app?.klassenbezeichnung?.trim() || 'nicht angegeben'} • SJ {app?.schuljahr?.trim() || 'nicht angegeben'}</span>
          </h1>
        </div>
        ...
        <div className="w-1/3 text-center px-4 border-x border-slate-200">
          ...
          <div className="text-[8pt] text-slate-600 mt-1 font-medium italic">Von Klassio erzeugte Druckansicht · vor Weitergabe prüfen</div>
        </div>
        ...
        <div className="w-1/3 text-right">
          ...
          <p className="font-bold text-black text-[10pt]">
            {[app?.anrede, app?.nachname].filter(Boolean).join(' ') || 'Lehrperson nicht angegeben'}
          </p>
          <p className="text-[9pt] text-slate-600">Stand: {new Date().toLocaleDateString('de-AT', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
        </div>
      </div>
    </div>
  );
